...
if [ -f "$GREEN_FILE" ]; then
  gunicorn config.wsgi -w 2 --daemon --bind 0.0.0.0:49153 --chdir $DONATE_ANYTHING_HOME_DIR
else
  if [ ! -f "$BLUE_FILE" ]; then
    ln -s /etc/nginx/sites-available/da-blue.conf /etc/nginx/sites-enabled
    service nginx reload
  fi
  gunicorn config.wsgi -w 2 --daemon --bind 0.0.0.0:49152 --chdir $DONATE_ANYTHING_HOME_DIR
fi
